...
class HarDNetSeg(nn.Module):
    def __init__(self, num_layers, heads, head_convs, opt, 
        ...
        # NEW lines for the base layer and previous image and heatmap
        ...
        for head in self.heads:
            ...
            # if head_conv > 0: # OLD line
            if len(head_convs) > 0: # NEW line
    ...
    # def forward(self, x): # OLD line
    # NEW lines
    def forward(self, x, pre_img=None, pre_hm=None):
        # NEW lines to combine current and previous image info (based off of DLA.forward in dla.py)
        x = self.base_layer(x)
        ...
        # for i in range(len(self.base)):   # OLD line
        for i in range(1, len(self.base)):  # NEW line
            x = self.base[i](x)
