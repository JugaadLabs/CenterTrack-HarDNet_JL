...
def get_engine(max_batch_size=1, onnx_file_path="", engine_file_path="", \
    ...
    def build_engine(max_batch_size):
        ...
        with trt.Builder(TRT_LOGGER) as builder, \
            ...
            # NEW two lines: (builder.max_workspace_size is removed in tensorrt 8)
            builder_config = builder.create_builder_config()
            builder_config.max_workspace_size = 1 << 30
            # builder.max_workspace_size = 1 << 30  # Your workspace size
            
            # NEW lines to set precision
            ...
            # NEW lines to build engine (builder.build_cuda_engine() was removed in tensorrt 8)
...
def demo(opt):
  ...
  if opt.load_trt:
    ...
    engine = get_engine(1, onnx_model_path, trt_engine_path, fp16_mode=True, int8_mode=False, overwrite=True)
  ...
  if opt.demo == 'webcam' or \
    ...
    for (image_name) in image_names:
      ...
      outs = do_inference(context, bindings=bindings, inputs=inputs, outputs=outputs, stream=stream)
      t_inf = perf_counter()-t_inf_start
      # print(outs)
      dets = outs[0].reshape(out_shape)
      # print(dets)
      show_det(dets, img, det_size, debugger, opt, True, os.path.basename(image_name))
      t_tot = perf_counter()-t_start

      print(f' Image: {os.path.basename(image_name)} | Timings: | tot {t_tot : .4f} | net {t_inf : .4f}')
